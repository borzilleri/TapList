...
A machine-readable JSON Schema for this shape is published at [`https://borzilleri.net/taplist-data/schema/dataset.schema.json`](https://borzilleri.net/taplist-data/schema/dataset.schema.json). Dataset authors can reference it via a top-level `"$schema": "https://borzilleri.net/taplist-data/schema/dataset.schema.json"` for IDE autocomplete and inline validation. The schema codifies the canonical contract; the runtime parser is intentionally more lenient on optional fields (see "Validation behavior" below).
...
| `styleCategory` | string | no       | Normalized style bucket for the list-view style filter, derived from the messy free-form `style`. One of a fixed enum (`IPA`, `Pale Ale`, `Lager & Pilsner`, `Wheat`, `Stout & Porter`, `Sour & Gose`, `Saison & Farmhouse`, `Belgian & Strong`, `Amber, Brown & Dark`, `Blonde, Kölsch & Cream`, `Cider`, `Wine & Mead`, `NA`, `Other`). `NA` collects non-alcoholic drinks (0% ABV, hop water, soda, N/A beers). Generated by `scripts/categorize-styles.ts`. A missing or unknown value is treated as `Other` at runtime. |
...
- `styleCategory` values outside the fixed enum (including missing) are treated as `null`; the list-view style filter buckets such beers under `Other`.
